...
/// arrival.
///
/// The span is measured on the entry's minute line (`entry_times.dart`), which
/// counts from midnight of the day it starts on. An entry that ends on a later
/// day therefore runs past 1439 rather than backwards: without that, a night
/// train departing 21:38 and arriving 02:25 had `end` below `start`, the guard
/// below flattened it to a *moment* at 21:38, and the traveller sitting on that
/// train was told the journey was behind them. It is also why a day *after* the
/// start has to shift its own clock onto that line before asking
/// ([continuationHappening] does).
TimeSpan? itemSpan(ItineraryItem item) {
  final times = item.times;
  final from = times.actualStart ?? times.plannedStart;
  final to = times.actualEnd ?? times.plannedEnd;
  final start = from ?? to;
  if (start == null) return null;
  final end = to ?? start;
  // A degenerate pair (an end before its start) is still read as a moment:
  // that is bad data, not a journey running backwards.
  return (start: start, end: end < start ? start : end);
}

/// Whether [item], seen from a day [daysAfter] days after the one it starts on,
/// is under way at [nowMinutes] of that day — the question a continuation row
/// asks on the morning after a night train left.
///
/// The clock is moved onto the entry's own minute line, where the span is
/// measured; an entry with no end there cannot reach into a later day at all.
bool continuationHappening(
  ItineraryItem item, {
  ...
  final now = daysAfter * kMinutesPerDay + nowMinutes;
  return span.end > span.start && now >= span.start && now < span.end;
}
