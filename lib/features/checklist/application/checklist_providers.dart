...
final checklistsProvider = StreamProvider.autoDispose
    .family<List<Checklist>, int>((ref, tripId) {
...
final checklistItemsProvider = StreamProvider.autoDispose
    .family<List<ChecklistItem>, int>((ref, checklistId) {
      return ref.watch(repositoryProvider).watchChecklistItems(checklistId);
    });

final checklistControllerProvider = Provider<ChecklistController>(
  (ref) => ChecklistController(ref),
);
...
class ChecklistController {
  ...
  Future<void> renameChecklist(Checklist checklist, String title) => _ref
      .read(repositoryProvider)
      .updateChecklist(checklist.copyWith(title: title.trim()));
  ...
  Future<void> setCollapsed(Checklist checklist, bool collapsed) => _ref
      .read(repositoryProvider)
      .updateChecklist(checklist.copyWith(collapsed: collapsed));
  ...
  Future<void> setDone(ChecklistItem item, bool done) => _ref
      .read(repositoryProvider)
      .updateChecklistItem(item.copyWith(done: done));

  Future<void> renameItem(ChecklistItem item, String label) => _ref
      .read(repositoryProvider)
      .updateChecklistItem(item.copyWith(label: label));
