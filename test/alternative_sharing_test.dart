...
void main() {
  ...
  Future<int> seedDecision(AppDatabase target) async {
    final tripId =
        await target.tripDao.createTrip(TripsCompanion.insert(title: 'Rome'));
    ...
    final branches =
        (await target.alternativeDao.watchBranchesForTrip(tripId).first)[setId]!;
    await target.alternativeDao.setAlternativeLabel(
      branches.first.id,
      'Museum day',
    );
    await target.costDao.addCost(CostsCompanion.insert(
      itemId: Value(museum),
      amountMinor: 1500,
  ...
  test('exportTrip captures the decision, its options and their items',
      () async {
    ...
    expect(item('Boat trip').alternativeLocalId, set.alternatives.last.localId);
    expect(item('Dinner').alternativeLocalId, isNull);

    // The option not taken travels with its price, or the recipient could not
    // compare the two.
    expect(bundle.costs.map((c) => c.amountMinor), unorderedEquals([1500, 5000]));
  });

  test('a trip with decisions is stamped v2, an ordinary one stays v1',
      () async {
    ...
    final plain = await db.tripDao.createTrip(TripsCompanion.insert(title: 'X'));
    expect((await db.sharingDao.exportTrip(plain))!.formatVersion, 1);
  });

  test('export then import reproduces the decision on the recipient\'s device',
      () async {
    ...
    expect(set.label, 'Saturday afternoon');
    final branches =
        (await db.alternativeDao.watchBranchesForTrip(newId).first)[set.id]!;
    expect(branches, hasLength(2));
    expect(branches.where((b) => b.chosen).map((b) => b.label), ['Museum day']);
  ...
  test('the recipient can switch to the option the sender did not take',
      () async {
    final source = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(source.close);
    final bytes =
        (await source.sharingDao.exportTrip(await seedDecision(source)))!.encode();
    final newId = await db.sharingDao.importTrip(TripBundle.decode(bytes));
  ...
  test('a group inside an option survives with its shared cost', () async {
    ...
    await source.costDao.addCost(CostsCompanion.insert(
    ...
    expect(legs.every((i) => i.alternativeId == legs.first.alternativeId), isTrue);
