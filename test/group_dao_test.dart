...
void main() {
  ...
  Future<int> makeItem(int tripId, {int sortOrder = 0}) => db.itineraryDao.addItem(
  ...
  test('groupItems extends an existing group when only one item is grouped',
      () async {
    final tripId = await makeTrip();
    final a = await makeItem(tripId, sortOrder: 0);
    final b = await makeItem(tripId, sortOrder: 1);
  ...
  test('groupItems merges two groups and re-points the second group costs',
      () async {
    final tripId = await makeTrip();
    final a = await makeItem(tripId, sortOrder: 0);
    final b = await makeItem(tripId, sortOrder: 1);
  ...
  test('dissolveGroup frees all members and preserves costs on the first',
      () async {
    final tripId = await makeTrip();
    final a = await makeItem(tripId, sortOrder: 0);
    final b = await makeItem(tripId, sortOrder: 1);
  ...
  test('deleteItem dissolves a 2-member group, preserving cost on the survivor',
      () async {
    final tripId = await makeTrip();
    final a = await makeItem(tripId, sortOrder: 0);
    final b = await makeItem(tripId, sortOrder: 1);
